...
#define array_size 1
unsigned long ReceivedSignal[array_size][2] = {{0, 0}};
...
boolean isAduplicate(long value) {
  Serial << "isAduplicate:" << value << "? " << endl;
  // check if the value has been already sent during the last time_avoid_duplicate
  for (int i = 0; i < array_size; i++) {
    Serial << "ReceivedSignal[i][0]:" << ReceivedSignal[i][0] << endl;
